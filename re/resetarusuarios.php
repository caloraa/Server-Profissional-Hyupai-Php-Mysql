...
<body>
    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content">
            <div class="animated fadeIn">
                ...
                <div class="col-lg-6">
                    <div class="card">
                        <div class="card-header">Resetar Usuario</div>
                        <div class="card-body card-block">
                            <form action="#" method="post" class="">
                                <div class="form-group">
                                    <div class="input-group">
                                        <div class="input-group-addon"><i class="fa fa-user"></i></div>
                                        <input type="text" id="username" name="username" placeholder="Usuario" class="form-control" data-required="true" data-error-message="Selecione uma opção" required>
                                    </div>
                                </div>
                            
                                <div class="form-actions form-group"><button type="submit" class="btn btn-success btn-medium" name="action">Resetar</button></div>
                            </form>
                        </div>
                    </div>
                      </div>
  ...
  <?php  if(isset($_POST['action'])) {
        ...
        if(mysqli_num_rows($resultadoa) == 0) {
                ...
                $update = "UPDATE `$tabela` SET `UID` = NULL Where `Username` = '$Username'";

$onupdate = mysqli_query($conn,$update);

 $update = "UPDATE `$tabela` SET `UID2` = NULL Where `Username` = '$Username'";

$onupdate = mysqli_query($conn,$update);

$name = '../logusers.txt';
$text = "$StartDate - O Usuario: $Username foi resetado por $Vendedor\n";
$file = fopen($name, 'a');
fwrite($file, $text);
fclose($file);
