    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content">
            <div class="animated fadeIn">
                ...
                <div class="col-lg-6">
                    <div class="card">
                        <div class="card-header">Banir Dispositivo</div>
                        <div class="card-body card-block">
                            <form action="#" method="post" class="">
                                ...
                                <div class="row form-group">
                                        <div class="col col-md-3"><label class=" form-control-label">Dispositivo (Device):</label></div>
                                        <div class="col col-md-9">
                                            <div class="form-check">
                                                <div class="radio">
                                                    <label for="radio1" class="form-check-label ">
                                                        <input type="radio" id="radio1" name="radios" value="1" class="form-check-input" data-required="true" data-error-message="Selecione uma opção" required > Banir o 1° Dispositivo
                                                    </label>
                                                </div>
                                                <div class="radio">
                                                    <label for="radio3" class="form-check-label ">
                                                        <input type="radio" id="radio3" name="radios" value="2" class="form-check-input" data-required="true" data-error-message="Selecione uma opção" required> Banir o 2° Dispositivo
                                                    </label>
                                                </div>
                                                <div class="radio">
                                                    <label for="radio4" class="form-check-label ">
                                                        <input type="radio" id="radio4" name="radios" value="3" class="form-check-input" data-required="true" data-error-message="Selecione uma opção" required> Banir todos os dispositivos
                                                    </label>
                                                </div>
                                            </div>
                                        </div>
                                ...
                                <div class="form-actions form-group"><button type="submit" class="btn btn-success btn-medium" name="action">Banir</button></div>
                            </form>
                        </div>
                    </div>
                      </div>
...
if ($devices == "3") {
    if($dev2 == NULL || $dev == NULL) {
        echo "<b> um dos dispositivo esta vazio!";
         die();
    }
}

$sqla = "SELECT device FROM $tabela4 WHERE user = '$Username'";
$resultadoa = mysqli_query($conn, $sqla); 

   if(mysqli_num_rows($resultadoa) > 0) {
                    echo "<b>Device ja esta banido!";
                    die();
                }

                    if ($devices == "1") {
                        $update = "INSERT INTO `$tabela4` (`user`, `device`) VALUES ('$Username', '$dev')";
                        $onupdate = mysqli_query($conn,$update);
                        $text = "$StartDate - O Usuario: $Username teve o dispositivo banido por $Vendedor\n";
                    } else if($devices == "2") {
                         $updatex = "INSERT INTO `$tabela4` (`user`, `device`) VALUES ('$Username', '$dev2')";
                        $onupdatex = mysqli_query($conn,$updatex);
                        $text = "$StartDate - O Usuario: $Username teve o dispositivo banido por $Vendedor\n";
                        
                    } else if($devices == "3") {
 $update = "INSERT INTO `$tabela4` (`user`, `device`) VALUES ('$Username', '$dev')";
                        $onupdate = mysqli_query($conn,$update);
                         $update2 = "INSERT INTO `$tabela4` (`user`, `device`) VALUES ('$Username', '$dev2')";
