    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content">
            <div class="animated fadeIn">
                ...
                <div class="col-lg-6">
                    <div class="card">
                        <div class="card-header">Editar Vendedores</div>
                        <div class="card-body card-block">
                            <form action="#" method="post" class="">
                                <div class="form-group">
                                    <div class="input-group">
                                        <div class="input-group-addon"><i class="fa fa-user"></i></div>
                                        <input type="text" id="username" name="usernamee" placeholder="Nome do Vendedor" class="form-control" data-required="true" required>
                                    </div>
                                </div>
                                 ...
                                 <div class="form-group">
                                    <div class="input-group">
                                        <div class="input-group-addon"><i class="fa fa-user"></i></div>
                                        <input type="text" id="username" name="username" placeholder="Novo Nome" class="form-control" data-required="true" required>
                                    </div>
                                </div>
                                ...
                                <p> Deixe em branco caso queira manter a mesma quantia de creditos!!</p>
                                <div class="form-group">
                                    <div class="input-group">
                                        <div class="input-group-addon"><i class="fa fa-key"></i></div>
                                        <input type="number" id="creditos" name="creditos" placeholder="Creditos" class="form-control">
                                    </div>
                                </div>
            ...
            $pass = md5($Senha);
            $Creditos2 = $_POST['creditos'];
            $Vendedor = $name;
            $StartDate= date('Y-m-d H:i');

            $cc4 = "SELECT * FROM $tabela3 WHERE $static = '$UsernameOld'";
$ccr4 = mysqli_query($conn, $cc4);
$dados4 = mysqli_fetch_array($ccr4);

$Creditos3 = $dados4['creditos'];

if ($Creditos2 == NULL || $Creditos2 == "") {
    $Creditos = $Creditos3;
} else {
    $Creditos = $Creditos2;
}

             $sqla = "SELECT $static FROM $tabela3 WHERE $static = '$UsernameOld'";
        ...
        if(mysqli_num_rows($resultadoa) == 0) {
             ...
             $update = "UPDATE $tabela3 SET `$static` = '$Username', `senha` = '$pass', `creditos` = '$Creditos' Where $static = '$UsernameOld'";
             $onupdate = mysqli_query($conn,$update);


     $a = 'ilogs.txt';
     $b = file_get_contents('ilogs.txt');
     $c = str_replace($UsernameOld,$Username,$b);
     file_put_contents($a, $c); 


$name = 'logusers.txt';
$text = "$StartDate - O vendedor $UsernameOld foi editado por $Vendedor\n";
$file = fopen($name, 'a');
fwrite($file, $text);
fclose($file);
