    ...
    <div id="right-panel" class="right-panel">
        ...
        <div class="content">
            <div class="animated fadeIn">
                ...
                <div class="col-lg-6">
                    <div class="card">
                        <div class="card-header">Adicionar Creditos</div>
                        <div class="card-body card-block">
                            <form action="#" method="post" class="">
                                <div class="form-group">
                                    <div class="input-group">
                                        <div class="input-group-addon"><i class="fa fa-user"></i></div>
                                        <input type="text" id="username" name="username" placeholder="Usuario" class="form-control" <?php $useer = $_GET['no']; if($_GET['no'] != NULL || $_GET['no'] != "") { echo "value=\"$useer\""; }?>data-required="true" required>
                                    </div>
                                </div>
                                <div class="form-group">
                                    <div class="input-group">
                                ...
                                <div class="form-actions form-group"><button type="submit" class="btn btn-success btn-medium" name="action">Adicionar Creditos</button></div>
                            </form>
                        </div>
                    </div>
                      </div>
...
$StartDate= date('Y-m-d H:i');
 ...
 $sqla = "SELECT $static FROM $tabela3 WHERE $static = '$Username'";
        ...
        if(mysqli_num_rows($resultadoa) == 0) {
                ...
                $sql12 = "SELECT * FROM $tabela3 WHERE $static = '$Username'";
    $resultado12 = mysqli_query($conn, $sql12);
    $dados12 = mysqli_fetch_array($resultado12);
   $Creditosold = $dados12['creditos'];

   $newC = $Creditosold + $Creditos;

            
                $update = "UPDATE $tabela3 SET `creditos` = '$newC' WHERE `$static` = '$Username'";

$onupdate = mysqli_query($conn,$update);

$name = 'logusers.txt';
$text = "$StartDate - foram adicionados $Creditos creditos à $Username por $Vendedor\n";
$file = fopen($name, 'a');
fwrite($file, $text);
fclose($file);
?>
